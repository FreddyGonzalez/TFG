...
export class PedidoComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    if (this.currentSearch) {
      this.pedidoService
        ...
        .subscribe(
          (res: HttpResponse<IPedido[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
          () => this.onError()
        );
      return;
    }
  ...
  registerChangeInPedidos(): void {
    this.eventSubscriber = this.eventManager.subscribe('pedidoListModification', () => this.loadPage());
  }
