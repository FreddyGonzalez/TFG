...
describe('Factura e2e test', () => {
  ...
  it('should load Facturas', async () => {
    ...
    expect(await facturaComponentsPage.getTitle()).to.eq('tiendaApp.facturaFactura.home.title');
  ...
  it('should load create Factura page', async () => {
    ...
    expect(await facturaUpdatePage.getPageTitle()).to.eq('tiendaApp.facturaFactura.home.createOrEditLabel');
  ...
  it('should create and save Facturas', async () => {
    ...
    await promise.all([
      facturaUpdatePage.setIdFacturaInput('idFactura'),
      ...
      facturaUpdatePage.setTotalInput('5'),
      facturaUpdatePage.setIdPedidoProductoInput('5'),
    ]);

    expect(await facturaUpdatePage.getIdFacturaInput()).to.eq('idFactura', 'Expected IdFactura value to be equals to idFactura');
    ...
    expect(await facturaUpdatePage.getTotalInput()).to.eq('5', 'Expected total value to be equals to 5');
    expect(await facturaUpdatePage.getIdPedidoProductoInput()).to.eq('5', 'Expected idPedidoProducto value to be equals to 5');
  ...
  it('should delete last Factura', async () => {
    ...
    expect(await facturaDeleteDialog.getDialogTitle()).to.eq('tiendaApp.facturaFactura.delete.question');
