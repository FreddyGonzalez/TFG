...
export class EnvioUpdatePage {
  ...
  idEnvioInput = element(by.id('field_idEnvio'));
  ...
  async setIdEnvioInput(idEnvio: string): Promise<void> {
    await this.idEnvioInput.sendKeys(idEnvio);
  }

  async getIdEnvioInput(): Promise<string> {
    return await this.idEnvioInput.getAttribute('value');
  }
