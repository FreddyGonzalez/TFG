...
describe('Pedido e2e test', () => {
  ...
  it('should load Pedidos', async () => {
    ...
    expect(await pedidoComponentsPage.getTitle()).to.eq('tiendaApp.productoPedido.home.title');
  ...
  it('should load create Pedido page', async () => {
    ...
    expect(await pedidoUpdatePage.getPageTitle()).to.eq('tiendaApp.productoPedido.home.createOrEditLabel');
  ...
  it('should create and save Pedidos', async () => {
    ...
    await promise.all([
      pedidoUpdatePage.setIdPedidoInput('idPedido'),
      pedidoUpdatePage.setFechaPedidoInput('01/01/2001' + protractor.Key.TAB + '02:30AM'),
      pedidoUpdatePage.estadoSelectLastOption(),
    ]);

    expect(await pedidoUpdatePage.getIdPedidoInput()).to.eq('idPedido', 'Expected IdPedido value to be equals to idPedido');
    expect(await pedidoUpdatePage.getFechaPedidoInput()).to.contain(
      '2001-01-01T02:30',
      'Expected fechaPedido value to be equals to 2000-12-31'
    );
  ...
  it('should delete last Pedido', async () => {
    ...
    expect(await pedidoDeleteDialog.getDialogTitle()).to.eq('tiendaApp.productoPedido.delete.question');
